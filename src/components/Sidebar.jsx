...
export default function Sidebar(props) {
  return (
    <div
      className={`sidebar position-fixed overflow-y-auto overflow-x-hidden transition ${
        !props.courseLayout ? "p-3  " : ""
      } ${props.sidebarShow ? "sidebar-show" : ""} `}
    >
      <div
        className={`sidbar-logo mb-5 d-flex align-items-center justify-content-center position-relative ${
          props.courseLayout ? "m-3" : ""
        } `}
      >
        <Link to="/dashboard">
      ...
      {!props.courseLayout ? (
        ...
        <div className="course-layout border border-2 ms-lg-4"><CourseSidebar/></div>
      )}
    </div>
  );
}
